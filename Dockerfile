...
# In Windows docker /c/windows/system32/config/system does not exist but is required by mesa systemwidedeploy.cmd -> create it temporarily
RUN "mkdir mesa && cd mesa && `
  ...
  mkdir /c/windows/system32/config/system && `
  cmd //c systemwidedeploy.cmd 1 && `
  rmdir /c/windows/system32/config/system && `
